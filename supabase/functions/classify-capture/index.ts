...
import { classifyCaptureWithGemini } from "./gemini_classify_client.ts";
...
async function readBody(req: Request): Promise<unknown> {
  ...
  if (contentLength && Number(contentLength) > MAX_BODY_BYTES) {
    throw mizAiError("INVALID_REQUEST", "classify request too large");
  }
  ...
  try {
    ...
    throw mizAiError("INVALID_REQUEST", "classify request is not valid JSON");
  }
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const request = parseClassifyCaptureRequest(await readBody(req));
    logEvent(requestId, "classify_request_started", {
      model,
    ...
    logEvent(requestId, "classify_request_completed", {
      success: true,
      kind: result.kind,
      durationMs: Date.now() - startedAt,
    });
    return Response.json({ success: true, result }, { headers: responseHeaders });
  } catch (error) {
